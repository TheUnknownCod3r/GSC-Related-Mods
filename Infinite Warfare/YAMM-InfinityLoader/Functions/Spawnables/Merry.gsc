...
*    Date : 13/09/2025 01:47:39
*
*/

spawn_merry( error )
{
    if( !(IsDefined( level.merry_spawned || level.SkybaseSpawned || level.TheClawSpawned || level.ferris_spawned)) )
    {
...
merrygoround()
{
    ...
    while( e < 3 )
    {
        ...
        while( i < 6 )
        {
            level.merrygoround[level.merrygoround.size] = modelspawner( level.merrygoroundmove.origin + ( cos( i * 60 ) * 20, sin( i * 60 ) * 20, e * 70 ), "care_package_iw7_dummy", ( 0, i * 60 + 90, 90 ), time );
            i++;
        }
        e++;
    }
    ...
    while( z < 2 )
    {
        ...
        while( e < 5 )
        {
            ...
            while( i < 6 * e )
            {
                level.merrygoround[level.merrygoround.size] = modelspawner( level.merrygoroundmove.origin + ( cos( i * ( 360 / ( 6 * e ) ) ) * int( sizes[ e] ), sin( i * ( 360 / ( 6 * e ) ) ) * int( sizes[ e] ), -20 + 190 * z ), "care_package_iw7_dummy", ( 0, i * ( 360 / ( 6 * e ) ) + 0, 0 ), time );
    ...
    while( e < 10 )
    {
        ...
        level.merrygoroundseats[level.merrygoroundseats.size] = modelspawner( level.merrygoroundmove.origin + ( cos( e * 36 ) * int( sizesseats[ e] ), sin( e * 36 ) * int( sizesseats[ e] ), 20 ), "care_package_iw7_dummy", ( 0, e * 36 + 0, 0 ), time );
